...
function WhatAreYouLookingFor({ onNext }) {
  ...
  return (
    <div style={datingBgStyle}>
      ...
      <div style={datingCardStyle} className="dating-card">
        ...
        <h2 style={datingTitleStyle}>What are you looking for?</h2>
        <p style={datingSubtitleStyle}>
          All good if it changes. There's something for everyone.
        </p>

        <div style={datingGridStyle}>
          {options.map((opt, index) => (
            <div
              key={index}
              onClick={() => selectOption(opt.label)}
              className="dating-option-card"
              style={{
                ...datingOptionCardStyle,
                backgroundColor: selected === opt.label ? 'rgba(255, 107, 157, 0.1)' : 'rgba(255, 255, 255, 0.8)',
                border: selected === opt.label ? '2px solid #ff6b9d' : '2px solid rgba(255, 107, 157, 0.2)',
                boxShadow: selected === opt.label ? '0 8px 24px rgba(255, 107, 157, 0.2)' : '0 2px 8px rgba(255, 107, 157, 0.1)',
              }}
            >
              <div style={datingEmojiStyle}>{opt.emoji}</div>
              <div style={{
                ...datingLabelStyle,
                color: selected === opt.label ? '#ff6b9d' : '#2d3748',
              }}>{opt.label}</div>
            </div>
          ))}
        </div>
